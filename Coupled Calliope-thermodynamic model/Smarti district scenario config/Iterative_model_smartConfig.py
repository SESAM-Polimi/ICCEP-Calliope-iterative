...
Calliope
'''
model_iter_0 = calliope.Model('model_op.yaml', scenario='zero_alt_storage')
model_iter_0.run()

T_dhw_st_0 = (model_iter_0.get_formatted_array('storage').loc[{'techs':'tes_dhw'}].to_pandas().T/C_dhw_r + 40)
T_sh_st_0 = (model_iter_0.get_formatted_array('storage').loc[{'techs':'tes_sh'}].to_pandas().T/C_sh_r + 25)
T_co_st_0 = (model_iter_0.get_formatted_array('storage').loc[{'techs':'tes_co'}].to_pandas().T/C_co_r + 5)
...
T = []
T.append(T_0)
T_dhw_r_0 = np.insert(T_dhw_st_0['buildings'].values,0,T_dhw)
T_sh_r_0 = np.insert(T_sh_st_0['buildings'].values,0,T_sh)
T_co_r_0 = np.insert(T_co_st_0['buildings'].values,0,T_co)
T_sh_c_0 = np.insert(T_sh_st_0['commercial'].values,0,T_sh)
T_co_c_0 = np.insert(T_co_st_0['commercial'].values,0,T_co)


Q_tot = ring_heat_tot_0.sum(axis=1).values*3600

for i in range(0,len(Q_tot)):
    if ((T[i]+(Q_tot[i])/(cp*Mw)) < Tmin):
        T.append(T_0)
    elif (T[i]+(Q_tot[i])/(cp*Mw)) > Tmax:
        T.append(T_0)
    else:
        T.append(T[i]+(Q_tot[i])/(cp*Mw))
       
    COP_dhw_series_r.append(6.81 - 0.121*(T_dhw_r_0[i]-T[i]) + 0.000630*math.pow((T_dhw_r_0[i]-T[i]),2))
    COP_sh_series_r.append(6.81 - 0.121*(T_sh_r_0[i]-T[i]) + 0.000630*math.pow((T_sh_r_0[i]-T[i]),2))
    COP_co_series_r.append(6.755 - 0.141*(T_co_r_0[i]-T[i]) + 0.003*math.pow((T_co_r_0[i]-T[i]),2))
    COP_sh_series_c.append(6.81 - 0.121*(T_sh_c_0[i]-T[i]) + 0.000630*math.pow((T_sh_c_0[i]-T[i]),2))
    COP_co_series_c.append(6.755 - 0.141*(T_co_c_0[i]-T[i]) + 0.003*math.pow((T_co_c_0[i]-T[i]),2))
...
while j <= 10 or np.nanmean(abs(T_ring_diff)) > 0.1: #np.nanmean(abs(T_ring_diff[np.where(abs(T_ring_diff)!= float('inf'))])) > 0.05: #
    ...
    Calliope
    '''
    model_iter = calliope.Model('model_op.yaml', scenario='different_storage')
    model_iter.run()
    
    T_dhw_st = (model_iter_0.get_formatted_array('storage').loc[{'techs':'tes_dhw'}].to_pandas().T/C_dhw_r + 40)
    T_sh_st = (model_iter_0.get_formatted_array('storage').loc[{'techs':'tes_sh'}].to_pandas().T/C_sh_r + 25)
    T_co_st = (model_iter_0.get_formatted_array('storage').loc[{'techs':'tes_co'}].to_pandas().T/C_co_r + 5)
    ...
    T = []
    T.append(T_0)
    T_dhw_r = np.insert(T_dhw_st['buildings'].values,0,T_dhw)
    T_sh_r = np.insert(T_sh_st['buildings'].values,0,T_sh)
    T_co_r = np.insert(T_co_st['buildings'].values,0,T_co)
    T_sh_c = np.insert(T_sh_st['commercial'].values,0,T_sh)
    T_co_c = np.insert(T_co_st['commercial'].values,0,T_co)

    
    Q_tot = ring_heat_tot.sum(axis=1).values*3600
    ...
    for i in range(0,len(Q_tot)):
        ...
        COP_dhw_series_r.append(6.81 - 0.121*(T_dhw_r[i]-T[i]) + 0.000630*math.pow((T_dhw_r[i]-T[i]),2))
        COP_sh_series_r.append(6.81 - 0.121*(T_sh_r[i]-T[i]) + 0.000630*math.pow((T_sh_r[i]-T[i]),2))
        COP_co_series_r.append(6.755 - 0.141*(T_co_r[i]-T[i]) + 0.003*math.pow((T_co_r[i]-T[i]),2))
        COP_sh_series_c.append(6.81 - 0.121*(T_sh_c[i]-T[i]) + 0.000630*math.pow((T_sh_c[i]-T[i]),2))
        COP_co_series_c.append(6.755 - 0.141*(T_co_c[i]-T[i]) + 0.003*math.pow((T_co_c[i]-T[i]),2))
